    ...
    case GameEvent.MeleeAttack(name, power, Some(id)) =>
      gameMap.entities.collectFirst {
        ...
        case Some(target) =>
          ...
          val msg =
            if damage > 0 then Message(s"${name.capitalize} attacks for $damage hit points.", RGB(1.0, 1.0, 1.0))
            else Message(s"${name.capitalize} attacks but does no damage", RGB(0.3, 0.3, 0.3))

          val res = gameMap.damageEntity(target.id, damage)
...
object Model:
  ...
  def generateModel(dice: Dice, screenSize: Size): Outcome[Model] =
    ...
    GameMap
      .generateMap(screenSize, dungeon)
